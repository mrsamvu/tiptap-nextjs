...
export const EmbedViewRender = ({ node, editor, getPos, selected }: NodeViewProps) => {
    ...
    return (
        ...
        >
            <div contentEditable={false} className={`relative w-full pb-[56.2061%] h-0 -m-[1px]`} onSelect={e => e.preventDefault()}>
                { !selected && isEditable && <div className='absolute z-20 w-full h-full hover:bg-gray-600/20'></div> }
                <iframe
                    src={node.attrs.src}
                    className={`absolute bg-black z-10 top-0 left-0 w-full h-full ${selected ? 'outline outline-2 outline-purple-600 outline-offset-[-2px]' : ''}`}
                    frameBorder="0"
                    allowFullScreen
                />
            </div>
